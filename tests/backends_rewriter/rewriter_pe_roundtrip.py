...
def parse_sections(path: pathlib.Path):
    ...
    names = []
    for i in range(num):
        name = data[sec_off + 40*i: sec_off + 40*i + 8].split(b'\0',1)[0].decode()
        names.append(name)
    return data, names
...
def main():
    ...
    data, names = parse_sections(out)
    if '.vmpload' not in names or '.CRT$XLB' not in names:
        raise SystemExit(f'missing expected sections: {names}')
