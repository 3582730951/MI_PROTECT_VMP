...
namespace vmp::tests::runtime_env_integrity {
...
class ScopedSignalHandler {
 public:
  ScopedSignalHandler(int signum, void (*handler)(int)) : signum_(signum) {
    previous_ = std::signal(signum_, handler);
  }

  ~ScopedSignalHandler() { std::signal(signum_, previous_); }
