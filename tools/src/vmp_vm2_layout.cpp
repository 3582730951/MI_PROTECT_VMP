...
Options parse_args(int argc, char** argv) {
  ...
  if (options.module_path.empty()) {
    throw std::runtime_error("usage: vmp-vm2-layout [--convert-to-reverse|--convert-to-forward] <module.vm2>");
  }
...
int main(int argc, char** argv) {
  try {
    ...
    auto module = vmp::runtime::vm2::Vm2Module::load_from_file(options.module_path);
    if (options.convert_to_reverse) {
      module.module_flags = static_cast<std::uint16_t>(module.module_flags | vmp::runtime::vm2::VMP_FLAG_REVERSE_ORDER);
      module.save_to_file(options.module_path);
      module = vmp::runtime::vm2::Vm2Module::load_from_file(options.module_path);
    ...
    const auto lengths = vmp::runtime::vm2::instruction_lengths(module);
    std::cout << "layout=" << (((module.module_flags & vmp::runtime::vm2::VMP_FLAG_REVERSE_ORDER) != 0u) ? "reverse" : "forward")
              << " instruction_count=" << lengths.size()
              << " length_table_checksum=0x" << std::hex << std::uppercase << length_checksum(lengths) << std::dec << "\n";
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "vmp-vm2-layout failed: " << ex.what() << '\n';
    return 1;
  }
}
