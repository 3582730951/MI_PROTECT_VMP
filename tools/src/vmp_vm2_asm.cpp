...
Options parse_args(int argc, char** argv) {
  ...
  if (options.input_path.empty() || options.output_path.empty()) {
    throw std::runtime_error("usage: vmp-vm2-asm [--encrypt-opcodes|--no-encrypt-opcodes] [--reverse-layout|--no-reverse-layout] [--opcode-seed <32-hex>] <input.vm2s> <output.vm2>");
  }
...
int main(int argc, char** argv) {
  try {
    ...
    if (options.reverse_layout) {
      assemble_options.module_flags = static_cast<std::uint16_t>(assemble_options.module_flags | vmp::runtime::vm2::VMP_FLAG_REVERSE_ORDER);
    }
